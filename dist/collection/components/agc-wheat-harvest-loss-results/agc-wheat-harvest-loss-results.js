export class AgcWheatHarvestLossResults {
    ...
    render() {
        return (h("section", { "data-wizard-results": true, ref: c => this.section = c },
            ...
            h("div", { style: { display: this.ready ? 'block' : 'none' } }, this.data && (h("ul", { class: "agc-results" },
                h("li", null,
                    h("h2", { "data-i18n": "results.wheat-yield-loss" }, "Wheat Yield Loss"),
                    h("span", { class: "agc-results__value" }, this.data['yieldLoss']),
                    h("sub", { "data-i18n": `results.yield-loss.${this.data['units']['yieldLoss']}` }, this.data['units']['yieldLoss'])))))));
    }
    handleResults(e) {
    ...
    static get is() { return "agc-wheat-harvest-loss-results"; }
